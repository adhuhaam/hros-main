...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check_eligibility'])) {
    ...
    $employee_query = $conn->prepare("SELECT gender, date_of_join FROM employees WHERE emp_no = ?");
    ...
    if (!$employee) {
        ...
        if ($start_date < $eligibility_date) {
            $error_msg = "Annual Leave eligibility starts from " . date('d-M-Y', strtotime($eligibility_date)) . ".";
        } else {
            // Fetch leave balance
            ...
            if ($balance < 1) {
                $error_msg = "You have zero balance left for Annual Leave.";
            } else {
                $success_msg = "Eligible for Annual Leave. You have $balance days available.";
                $is_eligible = true;
            }
        }
    }
...
<div class="page-wrapper">
    ...
    <div class="body-wrapper">
        ...
        <div class="container-fluid">
            ...
            <div class="card mt-4">
                <div class="card-body">
                    <h5 class="card-title fw-semibold mb-4">Annual Leave Application</h5>
                    ...
                    <?php if ($is_eligible) { ?>
                        <form method="POST" action="submit_annual_leave.php" class="shadow p-4 bg-light rounded mt-3">
                            ...
                            <button type="submit" class="btn btn-success w-100 mt-3">Submit Leave Application</button>
                        </form>
                    <?php } ?>
