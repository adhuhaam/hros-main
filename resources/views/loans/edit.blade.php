...
@section('title', 'Edit Loan Request - HR Management System')

@section('header', 'Edit Loan Request')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="employee_id" id="employee_id" required 
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->emp_no }}" {{ old('employee_id', $loan->employee_id) == $employee->emp_no ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="loan_type" id="loan_type" required 
                        ...
                        @foreach($loanTypes as $type)
                            <option value="{{ $type }}" {{ old('loan_type', $loan->loan_type) == $type ? 'selected' : '' }}>
                                {{ $type }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="total_installments" id="total_installments" value="{{ old('total_installments', $loan->total_installments) }}" min="1" required
                               class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                               placeholder="12">
                ...
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $loan->start_date->format('Y-m-d')) }}" required
                           class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    @error('start_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea name="purpose" id="purpose" rows="4" required
                              class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Please provide a detailed purpose for the loan request">{{ old('purpose', $loan->purpose) }}</textarea>
                    @error('purpose')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="guarantor_name" id="guarantor_name" value="{{ old('guarantor_name', $loan->guarantor_name) }}"
                               class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <div>
                        ...
                        <input type="text" name="guarantor_phone" id="guarantor_phone" value="{{ old('guarantor_phone', $loan->guarantor_phone) }}"
                               class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
