...
// Ensure only HR Manager can access
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $hrm_statement = $conn->real_escape_string($_POST['hrm_statement']);

    // Update the HRM comment and status in the warnings table
    $sql = "UPDATE warnings 
            SET hrm_statement = ?, status = 'Pending Director Review', updated_at = NOW() 
            WHERE id = ?";
    $stmt = $conn->prepare($sql);

    if ($stmt) {
        $stmt->bind_param("si", $hrm_statement, $warning_id);
        if ($stmt->execute()) {
            // Redirect to HRM's warnings list
...
<head>
    ...
    <title>Add Comment (HR Manager)</title>
            ...
            <header class="app-header">
                <nav class="navbar navbar-expand-lg navbar-light">
                    <div class="navbar-collapse justify-content-between">
                        <h3 class="fw-semibold mb-0 ms-6">Add Comment (HR Manager)</h3>
                        <a href="view_warnings.php" class="btn btn-primary">Back to Warnings</a>
                    </div>
                </nav>
            </header>
                ...
                <div class="card mt-4">
                    <div class="card-body">
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="mb-3">
                                <label for="hrm_statement" class="form-label">Comment:</label>
                                <textarea name="hrm_statement" id="hrm_statement" class="form-control" required></textarea>
                            </div>
